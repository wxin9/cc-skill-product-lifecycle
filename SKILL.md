...
# Product Lifecycle v2.1

**一句话**：脚本编排 + 交互暂停的产品全生命周期管理——orchestrator 自动执行 Phase 序列，遇到交互节点暂停通知模型，模型处理后 resume 继续。

**v2.1 新增**：Phase 1 实现方案分析——在项目初始化前，分析需求、项目代码、业界方案，生成多个方案供用户选择。Phase 编号调整为 1-11。
...
| Phase 4 | PRD 验证 + 自动快照 | 自动 | 验证 PRD 质量，通过后建快照 |
| Phase 5 | 架构访谈 + 项目类型识别 | **交互** | 用户回答 6 个访谈问题 |
| Phase 6 | AI 协作架构设计 | **交互** | Claude 生成架构草案，用户审核 + ADR 决策 |
| Phase 7 | 架构验证 + ADR 注册 + 快照 | 自动 | 验证架构文档，检查至少 1 条 ADR accepted |
| Phase 8 | 自适应测试大纲生成 | 自动 | 根据项目类型选择维度集，生成测试大纲 |
| Phase 9 | Velocity 感知迭代规划 | 自动 | 生成迭代计划，设定工时估算 |
| Phase 10 | 迭代执行循环 | **交互** | 用户开发、测试、通过 DoD 检查 |
| Phase 11 | 变更处理 | 自动 | 处理 PRD/Code/Test 变更，级联影响分析 |
...
# 2. Orchestrator 暂停在 Phase 1
#    通知: "等待用户选择实现方案"

# 3. 模型分析需求、项目代码、业界方案
#    生成多个方案供用户选择
#    写入 .lifecycle/solution.json
...
# 13. 模型调用 resume
./orchestrator resume --from-phase phase-5-arch-interview
...
# 2. Orchestrator 执行 Phase 11 (自动)
#    读取 PRD 快照，diff 识别变更，生成影响报告

# 3. Orchestrator 暂停在 Phase 3
#    通知: "等待用户修改 PRD"
...
# 6. Orchestrator 执行 Phase 4 (自动)
#    重新验证 PRD...

# 7. Orchestrator 执行 Phase 8-9 (自动)
#    更新测试大纲、迭代计划...
```
